...
|---|---|
| `halmasuit` binary | smithay-based Wayland server. Takes DRM master directly (`DRM_IOCTL_SET_MASTER`, not via logind). Starts in initramfs, survives `switch_root` via `@argv[0]` (validated by drm-master-probe), drops to `compositor` user via `setresuid`, hosts one foreground `wl_client` at a time. |
