//! SOCK_SEQPACKET framed transport + SO_PEERCRED peer primitive.
//!
//! Hard `#![forbid(unsafe_code)]` at the module level: every syscall
//! goes through `nix`'s safe wrappers. The crate's unsafe libpam FFI is
//! quarantined in `pam_ffi` and never reaches this module.
#![forbid(unsafe_code)]
