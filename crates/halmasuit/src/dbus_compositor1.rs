...
mod tests {
    ...
    /// The Compositor1 interface MUST expose EXACTLY the five read-only
    /// methods and nothing else — in particular no state-injection
    /// method (Epic #71 anti-pattern: NO Set*/Force*/Inject*/Override*).
    /// Parses zbus's real generated introspection XML (via the
    /// `Interface::introspect_to_writer` the `#[interface]` macro
    /// implements), so adding ANY method — injection or otherwise —
    /// fails this test, not just a name the test happened to enumerate.
    #[test]
    fn compositor1_interface_exposes_only_read_methods() {
        use zbus::object_server::Interface as _;

        let comp = Compositor1::new(CompositorObservability::new());
        let mut xml = String::new();
        comp.introspect_to_writer(&mut xml, 0);

        // Every `<method name="…">` in the introspection XML.
        let methods: Vec<&str> = xml
            .match_indices("<method name=\"")
            .map(|(i, m)| {
                let rest = &xml[i + m.len()..];
                &rest[..rest.find('"').expect("method name has a closing quote")]
            })
            .collect();
        ...
        assert_eq!(
            ...
            "Compositor1 must expose exactly the five read-only methods; got {methods:?}",
        );

        // Defense-in-depth: no state-injection verb survives even if the
        // exact-set assertion above is ever relaxed.
        for m in &methods {
            for verb in ["Set", "Force", "Inject", "Override"] {
                assert!(
                    !m.starts_with(verb),
                    "state-injection method {m:?} (starts with {verb:?}) is forbidden",
                );
            }
        }
    }
