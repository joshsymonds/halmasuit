...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    # beat later — with real niri the map→first-buffer gap is wide
    # enough to lose a race if we assert immediately. Wait for the
    # foreground_changed→session transition itself (state-based
    # polling, not a sleep) before asserting ordering.
    machine.wait_until_succeeds(
